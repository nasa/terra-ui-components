...
export function lockBodyScrolling(lockingEl: HTMLElement) {
  ...
  if (!document.documentElement.classList.contains('gd-scroll-lock')) {
    /** Scrollbar width + body padding calculation can go away once Safari has scrollbar-gutter support. */
    const scrollbarWidth = getScrollbarWidth() + getExistingBodyPadding(); // must be measured before the `gd-scroll-lock` class is applied
    document.documentElement.classList.add('gd-scroll-lock');
    document.documentElement.style.setProperty('--gd-scroll-lock-size', `${scrollbarWidth}px`);
  }
}
...
export function unlockBodyScrolling(lockingEl: HTMLElement) {
  ...
  if (locks.size === 0) {
    document.documentElement.classList.remove('gd-scroll-lock');
    document.documentElement.style.removeProperty('--gd-scroll-lock-size');
  }
}
