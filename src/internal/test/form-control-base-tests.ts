...
function runAllValidityTests(
  ...
  describe(`Form validity base test for ${displayName}`, async () => {
    ...
    it('should not emit an `gd-invalid` event when `.checkValidity()` is called while valid', async () => {
      const control = await createControl();
      const emittedEvents = checkEventEmissions(control, 'gd-invalid', () => control.checkValidity());
    ...
    it('should not emit an `gd-invalid` event when `.reportValidity()` is called while valid', async () => {
      const control = await createControl();
      const emittedEvents = checkEventEmissions(control, 'gd-invalid', () => control.reportValidity());
      expect(emittedEvents.length).to.equal(0);
    });
...
async function createFormControl<T extends GDFormControl = GDFormControl>(tagName: string): Promise<T> {
...
function checkEventEmissions(control: GDFormControl, eventType: string, action: () => void): Event[] {
