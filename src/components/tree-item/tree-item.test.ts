...
describe('<gd-tree-item>', () => {
  let leafItem: SlTreeItem;
  let parentItem: SlTreeItem;

  beforeEach(async () => {
    leafItem = await fixture(html` <gd-tree-item>Node 1</gd-tree-item> `);
  ...
  describe('when the user clicks the expand button', () => {
    describe('and the item is collapsed', () => {
      it('should emit gd-expand and gd-after-expand events', async () => {
        // Arrange
        const expandSpy = sinon.spy();
        const afterExpandSpy = sinon.spy();

        parentItem.addEventListener('gd-expand', expandSpy);
        parentItem.addEventListener('gd-after-expand', afterExpandSpy);
    ...
    describe('and the item is expanded', () => {
      it('should emit gd-collapse and gd-after-collapse events', async () => {
        // Arrange
        const collapseSpy = sinon.spy();
        const afterCollapseSpy = sinon.spy();

        parentItem.addEventListener('gd-collapse', collapseSpy);
        parentItem.addEventListener('gd-after-collapse', afterCollapseSpy);

        parentItem.expanded = true;
        await oneEvent(parentItem, 'gd-after-expand');
  ...
  describe('when the item is lazy', () => {
    it('should emit gd-lazy-change when the lazy attribute is added and removed', async () => {
      // Arrange
      const lazyChangeSpy = sinon.spy();

      parentItem.addEventListener('gd-lazy-change', lazyChangeSpy);
