...
import { registerIconLibrary } from '../../../dist/gesdisc-components.js';
...
describe('<gd-icon>', () => {
  ...
  describe('defaults ', () => {
    it('default properties', async () => {
      const el = await fixture<SlIcon>(html` <gd-icon></gd-icon> `);
    ...
    it('renders pre-loaded system icons and emits gd-load event', async () => {
      const el = await fixture<SlIcon>(html` <gd-icon library="system"></gd-icon> `);
      const listener = oneEvent(el, 'gd-load') as Promise<SlLoadEvent>;
    ...
    it('the icon is accessible', async () => {
      const el = await fixture<SlIcon>(html` <gd-icon library="system" name="check"></gd-icon> `);
  ...
  describe('when a label is provided', () => {
    it('the icon has the correct default aria attributes', async () => {
      ...
      const el = await fixture<SlIcon>(html` <gd-icon label="${fakeLabel}" library="system" name="check"></gd-icon> `);
  ...
  describe('when a valid src is provided', () => {
    it('the svg is rendered', async () => {
      ...
      const listener = oneEvent(el, 'gd-load');
  ...
  describe('new library', () => {
    it('renders icons from the new library and emits gd-load event', async () => {
      const el = await fixture<SlIcon>(html` <gd-icon library="test-library"></gd-icon> `);
      const listener = oneEvent(el, 'gd-load') as Promise<SlLoadEvent>;
    ...
    it('runs mutator from new library', async () => {
      const el = await fixture<SlIcon>(html` <gd-icon library="test-library" name="test-icon1"></gd-icon> `);
      await elementUpdated(el);
  ...
  describe('negative cases', () => {
    ...
    it("svg not rendered with an icon that doesn't exist in the library", async () => {
      const el = await fixture<SlIcon>(html` <gd-icon library="test-library" name="does-not-exist"></gd-icon> `);

      expect(el.shadowRoot?.querySelector('svg')).to.be.null;
    });

    it('emits gd-error when the file cant be retrieved', async () => {
      const el = await fixture<SlIcon>(html` <gd-icon library="test-library"></gd-icon> `);
      const listener = oneEvent(el, 'gd-error') as Promise<SlErrorEvent>;
    ...
    it("emits gd-error when there isn't an svg element in the registered icon", async () => {
      const el = await fixture<SlIcon>(html` <gd-icon library="test-library"></gd-icon> `);
      const listener = oneEvent(el, 'gd-error') as Promise<SlErrorEvent>;
  ...
  describe('svg sprite sheets', () => {
    ...
    it('Should properly grab an SVG and render it from bootstrap icons', async () => {
      ...
      const el = await fixture<SlIcon>(html`<gd-icon name="arrow-left" library="sprite"></gd-icon>`);
    ...
    it('Should render nothing if the sprite hash is wrong', async () => {
      ...
      const el = await fixture<SlIcon>(html`<gd-icon name="non-existent" library="sprite"></gd-icon>`);
      ...
      // https://github.com/gesdisc/components/issues/1925
      expect(svg?.getAttribute('fill')).to.equal('currentColor');
    });
    ...
    it.skip("Should produce an error if the icon doesn't exist.", async () => {
      ...
      const el = await fixture<SlIcon>(html`<gd-icon name="bad-icon" library="sprite"></gd-icon>`);
      const listener = oneEvent(el, 'gd-error') as Promise<SlErrorEvent>;
