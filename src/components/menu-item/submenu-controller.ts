...
export class SubmenuController implements ReactiveController {
  ...
  private addListeners() {
    ...
    if (!this.isPopupConnected) {
      if (this.popupRef.value) {
        ...
        this.popupRef.value.addEventListener('gd-reposition', this.handlePopupReposition);
  ...
  private removeListeners() {
    ...
    if (this.isPopupConnected) {
      if (this.popupRef.value) {
        ...
        this.popupRef.value.removeEventListener('gd-reposition', this.handlePopupReposition);
  ...
  private handleSubmenuEntry(event: KeyboardEvent) {
    ...
    for (const elt of submenuSlot.assignedElements()) {
      menuItems = elt.querySelectorAll("gd-menu-item, [role^='menuitem']");
  ...
  private handleClick = (event: MouseEvent) => {
    ...
    if (event.target === this.host) {
      ...
      (event.target.tagName === 'gd-menu-item' || event.target.role?.startsWith('menuitem'))
    ) {
      this.disableSubmenu();
    }
  };
  ...
  private handlePopupReposition = () => {
    const submenuSlot: HTMLSlotElement | null = this.host.renderRoot.querySelector("slot[name='submenu']");
    const menu = submenuSlot?.assignedElements({ flatten: true }).filter(el => el.localName === 'gd-menu')[0];
  ...
  renderSubmenu() {
    ...
    // Always render the slot, but conditionally render the outer <gd-popup>
    if (!this.isConnected) {
