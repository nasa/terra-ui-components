...
describe('<gd-menu-item>', () => {
  it('should pass accessibility tests', async () => {
    const el = await fixture<SlMenuItem>(html`
      <gd-menu>
        ...
        <gd-menu-item type="checkbox" checked>Checked</gd-menu-item>
        <gd-menu-item type="checkbox">Unchecked</gd-menu-item>
      </gd-menu>
    `);
      ...
      <gd-menu>
        <gd-menu-item>
          ...
          <gd-menu slot="submenu">
            <gd-menu-item>Submenu Item 1</gd-menu-item>
            <gd-menu-item>Submenu Item 2</gd-menu-item>
          </gd-menu>
        </gd-menu-item>
      </gd-menu>
    `);
  ...
  it('should have the correct default properties', async () => {
    const el = await fixture<SlMenuItem>(html` <gd-menu-item>Test</gd-menu-item> `);
  ...
  it('should render the correct aria attributes when disabled', async () => {
    const el = await fixture<SlMenuItem>(html` <gd-menu-item disabled>Test</gd-menu-item> `);
    expect(el.getAttribute('aria-disabled')).to.equal('true');
  });

  describe('when loading', () => {
    it('should have a spinner present', async () => {
      const el = await fixture<SlMenuItem>(html` <gd-menu-item loading>Menu Item Label</gd-menu-item> `);
      expect(el.shadowRoot!.querySelector('gd-spinner')).to.exist;
    });
  });
  ...
  it('should emit the slotchange event when the label changes', async () => {
    const el = await fixture<SlMenuItem>(html` <gd-menu-item>Text</gd-menu-item> `);
  ...
  it('should render a hidden menu item when the inert attribute is used', async () => {
    const menu = await fixture<SlMenuItem>(html`
      <gd-menu>
        <gd-menu-item inert>Item 1</gd-menu-item>
        <gd-menu-item>Item 2</gd-menu-item>
        <gd-menu-item>Item 3</gd-menu-item>
      </gd-menu>
    `);
    const item1 = menu.querySelector('gd-menu-item')!;

    expect(getComputedStyle(item1).display).to.equal('none');
  });

  it('should not render a gd-popup if the slot="submenu" attribute is missing, but the slot should exist in the component and be hidden.', async () => {
    const menu = await fixture<SlMenuItem>(html`
      <gd-menu>
        <gd-menu-item>
          Item 1
          <gd-menu>
            <gd-menu-item> Nested Item 1 </gd-menu-item>
          </gd-menu>
        </gd-menu-item>
      </gd-menu>
    `);

    const menuItem: HTMLElement = menu.querySelector('gd-menu-item')!;
    expect(menuItem.shadowRoot!.querySelector('gd-popup')).to.be.null;
    const submenuSlot: HTMLElement = menuItem.shadowRoot!.querySelector('slot[name="submenu"]')!;
    expect(submenuSlot.hidden).to.be.true;
  });

  it('should render an gd-popup if the slot="submenu" attribute is present', async () => {
    const menu = await fixture<SlMenuItem>(html`
      <gd-menu>
        <gd-menu-item id="test">
          Item 1
          <gd-menu slot="submenu">
            <gd-menu-item> Nested Item 1 </gd-menu-item>
          </gd-menu>
    ...
    const menuItem = menu.querySelector('gd-menu-item')!;
    expect(menuItem.shadowRoot!.querySelector('gd-popup')).to.be.not.null;
    const submenuSlot: HTMLElement = menuItem.shadowRoot!.querySelector('slot[name="submenu"]')!;
  ...
  it('should focus on first menuitem of submenu if ArrowRight is pressed on parent menuitem', async () => {
    const menu = await fixture<SlMenuItem>(html`
      <gd-menu>
        <gd-menu-item id="item-1">
          Submenu
          <gd-menu slot="submenu">
            <gd-menu-item value="submenu-item-1"> Nested Item 1 </gd-menu-item>
          </gd-menu>
        </gd-menu-item>
      </gd-menu>
    `);
    ...
    const submenu = menu.querySelector('gd-menu-item');
    submenu!.focus();
  ...
  it('should focus on outer menu if ArrowRight is pressed on nested menuitem', async () => {
    const menu = await fixture<SlMenuItem>(html`
      <gd-menu>
        <gd-menu-item value="outer-item-1">
          Submenu
          <gd-menu slot="submenu">
            <gd-menu-item value="inner-item-1"> Nested Item 1 </gd-menu-item>
          </gd-menu>
        </gd-menu-item>
      </gd-menu>
    `);
    ...
    const outerItem = menu.querySelector('gd-menu-item');
    outerItem!.focus();
    await menu.updateComplete;
    await sendKeys({ press: 'ArrowRight' });
