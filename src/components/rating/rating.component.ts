 ...
 * @documentation https://disc.gsfc.nasa.gov/components/rating
 * @status stable
 * @since 2.0
 *
 * @dependency gd-icon
 *
 * @event gd-change - Emitted when the rating's value changes.
 * @event {{ phase: 'start' | 'move' | 'end', value: number }} gd-hover - Emitted when the user hovers over a value. The
 ...
 */
export default class SlRating extends GDElement {
   ...
   * well with `<gd-icon>` elements.
   */
  @property() getSymbol: (value: number) => string = () => '<gd-icon name="star-fill" library="system"></gd-icon>';
