...
describe('<gd-tab-panel>', () => {
  it('passes accessibility test', async () => {
    const el = await fixture<SlTabPanel>(html` <gd-tab-panel>Test</gd-tab-panel> `);
  ...
  it('default properties', async () => {
    ...
    expect(el.id).to.equal('gd-tab-panel-2');
  ...
  it('passed id should be used', async () => {
    const el = await fixture<SlTabPanel>(html` <gd-tab-panel id="test-id">Test</gd-tab-panel> `);
