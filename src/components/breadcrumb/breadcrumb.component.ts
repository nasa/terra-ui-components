 ...
 * @documentation https://disc.gsfc.nasa.gov/components/breadcrumb
 * @status stable
 * @since 2.0
 *
 * @slot - One or more breadcrumb items to display.
 * @slot separator - The separator to use between breadcrumb items. Works best with `<gd-icon>`.
 *
 * @dependency gd-icon
 *
 * @csspart base - The component's base wrapper.
 */
export default class SlBreadcrumb extends GDElement {
  ...
  private handleSlotChange() {
    const items = [...this.defaultSlot.assignedElements({ flatten: true })].filter(
      item => item.tagName.toLowerCase() === 'gd-breadcrumb-item'
    ) as SlBreadcrumbItem[];
  ...
  render() {
    ...
    return html`
      ...
      <span hidden aria-hidden="true">
        <slot name="separator">
          <gd-icon name=${this.localize.dir() === 'rtl' ? 'chevron-left' : 'chevron-right'} library="system"></gd-icon>
        </slot>
      </span>
    `;
  }
