...
describe('<gd-dialog>', () => {
  it('should be visible with the open attribute', async () => {
    const el = await fixture<SlDialog>(html`
      <gd-dialog open>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</gd-dialog>
    `);
  ...
  it('should not be visible without the open attribute', async () => {
    const el = await fixture<SlDialog>(html`
      <gd-dialog>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</gd-dialog>
    `);
    const base = el.shadowRoot!.querySelector<HTMLElement>('[part~="base"]')!;
  ...
  it('should allow initial focus to be set', async () => {
    const el = await fixture<SlDialog>(html` <gd-dialog><input /></gd-dialog> `);
  ...
  it('should close when pressing Escape', async () => {
    const el = await fixture<SlDialog>(html` <gd-dialog open></gd-dialog> `);
    const hideHandler = sinon.spy();
  ...
  // https://github.com/gesdisc/components/issues/1382
  it('should properly cycle through tabbable elements when gd-dialog is used in a shadowRoot', async () => {
    class AContainer extends LitElement {
      get dialog() {
        return this.shadowRoot?.querySelector('gd-dialog');
      }
      ...
      render() {
        return html`
          ...
          <gd-button @click=${this.openDialog}>Open Dialog</gd-button>
        `;
      }
    }
    ...
    const dialog = container.shadowRoot?.querySelector('gd-dialog');

    if (!dialog) {
      throw Error('Could not find <gd-dialog> element.');
    }
    ...
    // Opens modal.
    const openModalButton = container.shadowRoot?.querySelector('gd-button');
