...
const expectHideAndAfterHideToBeEmittedInCorrectOrder = async (alert: SlAlert, action: () => void | Promise<void>) => {
  const hidePromise = oneEvent(alert, 'gd-hide');
  const afterHidePromise = oneEvent(alert, 'gd-after-hide');
  let afterHideHappened = false;
  oneEvent(alert, 'gd-after-hide').then(() => (afterHideHappened = true));
...
const expectShowAndAfterShowToBeEmittedInCorrectOrder = async (alert: SlAlert, action: () => void | Promise<void>) => {
  const showPromise = oneEvent(alert, 'gd-show');
  const afterShowPromise = oneEvent(alert, 'gd-after-show');
  let afterShowHappened = false;
  oneEvent(alert, 'gd-after-show').then(() => (afterShowHappened = true));
...
describe('<gd-alert>', () => {
  ...
  it('renders', async () => {
    const alert = await fixture<SlAlert>(html`<gd-alert open>I am an alert</gd-alert>`);
  ...
  describe('alert visibility', () => {
    ...
    it('should not be visible without the open attribute', async () => {
      const alert = await fixture<SlAlert>(html` <gd-alert>I am an alert</gd-alert>`);
  ...
  describe('close button', () => {
    it('shows a close button if the alert has the closable attribute', () => async () => {
      const alert = await fixture<SlAlert>(html` <gd-alert open closable>I am an alert</gd-alert> `);
  ...
  describe('toast', () => {
    const getToastStack = (): HTMLDivElement | null => document.querySelector<HTMLDivElement>('.gd-toast-stack');
    ...
    it('resolves only after being closed', async () => {
      const alert = await fixture<SlAlert>(html`<gd-alert closable>I am an alert</gd-alert>`);

      const afterShowEvent = oneEvent(alert, 'gd-after-show');
      ...
      const closePromise = oneEvent(alert, 'gd-after-hide');
      const closeButton = getCloseButton(alert);
      clickOnElement(closeButton!);
    ...
    const openToast = async (alert: SlAlert): Promise<void> => {
      const openPromise = oneEvent(alert, 'gd-after-show');
      alert.toast();
      await openPromise;
    };
    ...
    it('deletes the toast stack after the last alert is done', async () => {
      const container = await fixture<HTMLElement>(
        html`<div>
          <gd-alert data-testid="alert1" closable>alert 1</gd-alert>
          <gd-alert data-testid="alert2" closable>alert 2</gd-alert>
        </div>`
      );
  ...
  describe('timer controlled closing', () => {
    it('closes after a predefined amount of time', async () => {
      ...
      const alert = await fixture<SlAlert>(html` <gd-alert open duration="3000">I am an alert</gd-alert>`);
    ...
    it('resets the closing timer after opening', async () => {
      clock = sinon.useFakeTimers();
      const alert = await fixture<SlAlert>(html` <gd-alert duration="3000">I am an alert</gd-alert>`);
  ...
  describe('alert variants', () => {
    ...
    variants.forEach(variant => {
      it(`adapts to the variant: ${variant}`, async () => {
        const alert = await fixture<SlAlert>(html`<gd-alert variant="${variant}" open>I am an alert</gd-alert>`);
