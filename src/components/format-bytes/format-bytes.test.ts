...
describe('<gd-format-bytes>', () => {
  describe('defaults ', () => {
    it('default properties', async () => {
      const el = await fixture<SlFormatBytes>(html` <gd-format-bytes></gd-format-bytes> `);
  ...
  describe('bits', () => {
    ...
    results.forEach(expected => {
      it('bits : display formats', async () => {
        const el = await fixture<SlFormatBytes>(html` <gd-format-bytes unit="bit"></gd-format-bytes> `);
        // short
        el.value = expected.value;
        await elementUpdated(el);
        expect(el.shadowRoot?.textContent).to.equal(expected.short);
