...
describe('<gd-tab>', () => {
  ...
  it('should render default tab', async () => {
    const el = await fixture<SlTab>(html` <gd-tab>Test</gd-tab> `);
  ...
  it('should disable tab by attribute', async () => {
    const el = await fixture<SlTab>(html` <gd-tab disabled>Test</gd-tab> `);
  ...
  it('should set active tab by attribute', async () => {
    const el = await fixture<SlTab>(html` <gd-tab active>Test</gd-tab> `);
  ...
  it('should set closable by attribute', async () => {
    const el = await fixture<SlTab>(html` <gd-tab closable>Test</gd-tab> `);
  ...
  describe('closable', () => {
    it('should emit close event when the close button is clicked', async () => {
      const tabGroup = await fixture<SlTabGroup>(html`
        <gd-tab-group>
          <gd-tab slot="nav" panel="general" closable>General</gd-tab>
          <gd-tab slot="nav" panel="custom" closable>Custom</gd-tab>
      ...
      const closeButton = tabGroup
        .querySelectorAll('gd-tab')[0]!
        .shadowRoot!.querySelector<SlIconButton>('[part~="close-button"]')!;

      const handleClose = sinon.spy();
      const handleTabShow = sinon.spy();

      tabGroup.addEventListener('gd-close', handleClose, { once: true });
      // The gd-tab-show event shouldn't be emitted when clicking the close button
      tabGroup.addEventListener('gd-tab-show', handleTabShow);
