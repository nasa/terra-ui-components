		...
		this.giovanniConfiguredVariablesTask = new Task(host, {
			task: async ([], { signal }) => {
				try {
					...
					console.warn('Error fetching Giovanni configured variables:', error)
		...
		this.jobStatusTask = new Task(host, {
			task: async ([], { signal }) => {
				...
				if (this.currentJob?.jobID) {
					...
					const variables = isGiovanniFormat
						? this.#host.selectedVariables.map((v) =>
								...
								convertVariableEntryIdToGiovanniFormat(
									this.#host.collectionWithServices?.shortName ?? '',
									this.#host.collectionWithServices?.collection?.Version ?? '',
									v.name,
								),
							)
						: this.#host.selectedVariables.map((v) => v.conceptId)

					const harmonyRequest = new HarmonyRequest({
						collectionConceptId: this.#host.collectionWithServices!.conceptId,
						variables,
						location: this.#host.spatialSelection,
					})
					...
					// create the new job
					try {
						job = await this.#dataService.createSubsetJob(harmonyRequest, {
							signal,
							bearerToken: this.#host.bearerToken,
							environment: this.#host.environment,
						})
