...
export default class TerraTimeAverageMap extends TerraElement {
  ...
  @property({ attribute: 'bearer-token', reflect: false }) bearerToken: string
  @property({ type: String }) long_name = ''

  // Query and State
  @query('#menu') menu: HTMLMenuElement
  @state() activeMenuItem: any = null
  @state() colormaps = [
    'jet', 'hsv', 'hot', 'cool', 'spring', 'summer', 'autumn', 'winter', 'bone', 'copper',
    'greys', 'YIGnBu', 'greens', 'YIOrRd', 'bluered', 'RdBu', 'picnic', 'rainbow', 'portland',
    'blackbody', 'earth', 'electric', 'viridis', 'inferno', 'magma', 'plasma', 'warm', 'cool',
    'bathymetry', 'cdom', 'chlorophyll', 'density', 'fressurface-blue', 'freesurface-red',
    'oxygen', 'par', 'phase', 'salinity', 'temperature', 'turbidity', 'velocity-blue',
    'velocity-green', 'cubhelix'
  ]
  ...
  async updateGeoTIFFLayer() {
    ...
    if (this.#map && this.#gtLayer) {
      ...
      const opacityInput = this.shadowRoot?.getElementById('opacity-input') as HTMLInputElement | null
      const opacityOutput = this.shadowRoot?.getElementById('opacity-output') as HTMLElement | null
  ...
  async getMinMax(gtSource: any) {
    ...
    for (let i = 0; i < pixels.length; i++) {
      const val = pixels[i]
      if (!isNaN(val)) { // skip no-data pixels or NaN
        if (val < min) min = val
        if (val > max) max = val
      }
    }
