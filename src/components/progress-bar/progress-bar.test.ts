...
describe('<gd-progress-bar>', () => {
  let el: SlProgressBar;

  describe('when provided just a value parameter', () => {
    before(async () => {
      el = await fixture<SlProgressBar>(html`<gd-progress-bar value="25"></gd-progress-bar>`);
    });
  ...
  describe('when provided a title, and value parameter', () => {
    ...
    before(async () => {
      el = await fixture<SlProgressBar>(
        html`<gd-progress-bar title="Titled Progress Ring" value="25"></gd-progress-bar>`
      );
  ...
  describe('when provided an indeterminate parameter', () => {
    ...
    before(async () => {
      el = await fixture<SlProgressBar>(
        html`<gd-progress-bar title="Titled Progress Ring" indeterminate></gd-progress-bar>`
      );
      base = el.shadowRoot!.querySelector('[part~="base"]')!;
    });
  ...
  describe('when provided a ariaLabel, and value parameter', () => {
    before(async () => {
      el = await fixture<SlProgressBar>(
        html`<gd-progress-bar ariaLabel="Labelled Progress Ring" value="25"></gd-progress-bar>`
      );
    });
  ...
  describe('when provided a ariaLabelledBy, and value parameter', () => {
    before(async () => {
      el = await fixture<SlProgressBar>(html`
        ...
        <gd-progress-bar ariaLabelledBy="labelledby" value="25"></gd-progress-bar>
      `);
    });
