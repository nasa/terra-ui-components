...
describe('<gd-qr-code>', () => {
  it('should render a component', async () => {
    const qrCode = await fixture<SlQrCode>(html` <gd-qr-code value="test data"></gd-qr-code>`);
  ...
  it('uses the label if given', async () => {
    const qrCode = await fixture<SlQrCode>(html` <gd-qr-code value="test data" label="test label"></gd-qr-code>`);

    expectCanvasToHaveAriaLabel(qrCode, 'test label');
  });

  it('sets the correct color for the qr code', async () => {
    const qrCode = await fixture<SlQrCode>(html` <gd-qr-code value="test data" fill="red"></gd-qr-code>`);
  ...
  it('sets the correct background for the qr code', async () => {
    const qrCode = await fixture<SlQrCode>(
      html` <gd-qr-code value="test data" fill="red" background="blue"></gd-qr-code>`
    );
  ...
  it('has the expected size', async () => {
    const qrCode = await fixture<SlQrCode>(html` <gd-qr-code value="test data" size="100"></gd-qr-code>`);
