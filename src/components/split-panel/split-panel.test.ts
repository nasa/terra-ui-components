...
describe('<gd-split-panel>', () => {
  afterEach(async () => {
    await resetMouse();
  });

  it('should render a component', async () => {
    const splitPanel = await fixture(html` <gd-split-panel></gd-split-panel> `);
  ...
  describe('panel sizing horizontal', () => {
    ...
    it('changes the sizing of the panels based on the position attribute', async () => {
      const splitPanel = await fixture<SlSplitPanel>(
        html`<gd-split-panel position="25">
          <div slot="start" data-testid="start-panel">Start</div>
          <div slot="end" data-testid="end-panel">End</div>
        </gd-split-panel>`
      );
    ...
    it('emits the gd-reposition	event on position change', async () => {
      const splitPanel = await fixture<SlSplitPanel>(
        html`<gd-split-panel>
          <div slot="start">Start</div>
          <div slot="end">End</div>
      ...
      const repositionPromise = oneEvent(splitPanel, 'gd-reposition');
      splitPanel.position = 10;
  ...
  describe('panel sizing vertical', () => {
    it('has two evenly sized panels by default', async () => {
      const splitPanel = await fixture<SlSplitPanel>(
        html`<gd-split-panel vertical style="height: 400px;">
          <div slot="start" data-testid="start-panel">Start</div>
          <div slot="end" data-testid="end-panel">End</div>
        </gd-split-panel>`
      );
    ...
    it('changes the sizing of the panels based on the position attribute', async () => {
      const splitPanel = await fixture<SlSplitPanel>(
        html`<gd-split-panel position="25" vertical style="height: 400px;">
          <div slot="start" data-testid="start-panel">Start</div>
          <div slot="end" data-testid="end-panel">End</div>
        </gd-split-panel>`
      );
    ...
    it('cannot be resized if disabled', async () => {
      const splitPanel = await fixture<SlSplitPanel>(
        html`<gd-split-panel disabled vertical style="height: 400px;">
          <div slot="start">Start</div>
          <div slot="end">End</div>
        </gd-split-panel>`
      );
