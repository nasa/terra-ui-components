...
import { BordersLayer } from '../borders.layer.js'

describe('BordersLayer', () => {
    it('should create a VectorTileLayer with a VectorTileSource', () => {
        const layer = new BordersLayer()
    ...
    it('should propagate noWorldWrap to wrapX: false on the source', () => {
        const layer = new BordersLayer({ noWorldWrap: true })
        const source = layer.getSource() as VectorTileSource
    ...
    it('should apply a style function derived from the borders style config', () => {
        const layer = new BordersLayer()
