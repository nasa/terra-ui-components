 ...
 * @documentation https://disc.gsfc.nasa.gov/components/color-picker
 * @status stable
 * @since 2.0
 *
 * @dependency gd-button
 * @dependency gd-button-group
 * @dependency gd-dropdown
 * @dependency gd-input
 * @dependency gd-visually-hidden
 *
 * @slot label - The color picker's form label. Alternatively, you can use the `label` attribute.
 *
 * @event gd-blur - Emitted when the color picker loses focus.
 * @event gd-change - Emitted when the color picker's value changes.
 * @event gd-focus - Emitted when the color picker receives focus.
 * @event gd-input - Emitted when the color picker receives input.
 ...
 */
export default class SlColorPicker extends GDElement implements GDFormControl {
  ...
  private handleInputChange(event: SlChangeEvent) {
    ...
    // Prevent the <gd-input>'s gd-change event from bubbling up
  ...
  private handleInputInput(event: SlInputEvent) {
    ...
    // Prevent the <gd-input>'s gd-input event from bubbling up
    event.stopPropagation();
  }
  ...
  reportValidity() {
    if (!this.inline && !this.validity.valid) {
      ...
      this.dropdown.show();
      this.addEventListener('gd-after-show', () => this.input.reportValidity(), { once: true });

      if (!this.disabled) {
        // By standards we have to emit a `gd-invalid` event here synchronously.
  ...
  render() {
      ...
      >
        ...
        <div class="color-picker__user-input" aria-live="polite">
          <gd-input
            ...
            @gd-change=${this.handleInputChange}
            @gd-input=${this.handleInputInput}
            @gd-invalid=${this.handleInputInvalid}
            @gd-blur=${this.stopNestedEventPropagation}
            @gd-focus=${this.stopNestedEventPropagation}
          ></gd-input>
    ...
    return html`
      <gd-dropdown
        ...
        ?disabled=${this.disabled}
        ?hoist=${this.hoist}
        @gd-after-hide=${this.handleAfterHide}
      >
        <button
          part="trigger"
          slot="trigger"
