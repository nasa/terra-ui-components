...
describe('<gd-color-picker>', () => {
  describe('when the value changes', () => {
    it('should not emit gd-change or gd-input when the value is changed programmatically', async () => {
      const el = await fixture<SlColorPicker>(html` <gd-color-picker></gd-color-picker> `);
      const color = 'rgb(255, 204, 0)';

      el.addEventListener('gd-change', () => expect.fail('gd-change should not be emitted'));
      el.addEventListener('gd-input', () => expect.fail('gd-change should not be emitted'));
      el.value = color;
      await el.updateComplete;
    });

    it('should emit gd-change and gd-input when the color grid selector is moved', async () => {
      const el = await fixture<SlColorPicker>(html` <gd-color-picker></gd-color-picker> `);
      ...
      // Simulate a drag event. "gd-change" should not fire until we stop dragging.
    ...
    it('should emit gd-change and gd-input when the hue slider is moved', async () => {
      const el = await fixture<SlColorPicker>(html` <gd-color-picker></gd-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit gd-change and gd-input when the opacity slider is moved', async () => {
      const el = await fixture<SlColorPicker>(html` <gd-color-picker opacity></gd-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit gd-change and gd-input when toggling the format', async () => {
      const el = await fixture<SlColorPicker>(html` <gd-color-picker value="#fff"></gd-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should render the correct swatches when passing a string of color values', async () => {
      const el = await fixture<SlColorPicker>(html`
        <gd-color-picker swatches="red; #008000; rgb(0,0,255);"></gd-color-picker>
      `);
    ...
    it('should emit gd-change and gd-input when clicking on a swatch', async () => {
      const el = await fixture<SlColorPicker>(html` <gd-color-picker swatches="red; green; blue;"></gd-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit gd-change and gd-input when selecting a color with the keyboard', async () => {
      const el = await fixture<SlColorPicker>(html` <gd-color-picker></gd-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit gd-change and gd-input when selecting hue with the keyboard', async () => {
      const el = await fixture<SlColorPicker>(html` <gd-color-picker></gd-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit gd-change and gd-input when selecting opacity with the keyboard', async () => {
      const el = await fixture<SlColorPicker>(html` <gd-color-picker opacity></gd-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit gd-change and gd-input when entering a value in the color input and pressing enter', async () => {
      const el = await fixture<SlColorPicker>(html` <gd-color-picker opacity></gd-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit gd-change and gd-input when entering a value in the color input and blurring the field', async () => {
      const el = await fixture<SlColorPicker>(html` <gd-color-picker opacity></gd-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
      const input = el.shadowRoot!.querySelector<HTMLElement>('[part~="input"]')!;
      const changeHandler = sinon.spy();
    ...
    it('should render the correct format when selecting a swatch of a different format', async () => {
      const el = await fixture<SlColorPicker>(html` <gd-color-picker format="rgb"></gd-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
      const changeHandler = sinon.spy();
      const inputHandler = sinon.spy();
  ...
  it('should render in a dropdown', async () => {
    const el = await fixture<SlColorPicker>(html` <gd-color-picker></gd-color-picker> `);
    const dropdown = el.shadowRoot!.querySelector('gd-dropdown');

    expect(dropdown).to.exist;
  });

  it('should not render in a dropdown when inline is enabled', async () => {
    const el = await fixture<SlColorPicker>(html` <gd-color-picker inline></gd-color-picker> `);
    const dropdown = el.shadowRoot!.querySelector('gd-dropdown');
  ...
  it('should display a color when an initial value is provided', async () => {
    const el = await fixture<SlColorPicker>(html` <gd-color-picker value="#000"></gd-color-picker> `);
    const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]');
  ...
  it('should display a color with opacity when an initial value with opacity is provided', async () => {
    const el = await fixture<SlColorPicker>(html` <gd-color-picker opacity value="#ff000050"></gd-color-picker> `);
    const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
  ...
  it.skip('should emit gd-focus when rendered as a dropdown and focused', async () => {
    const el = await fixture<SlColorPicker>(html`
      <div>
    ...
    `);
    const colorPicker = el.querySelector('gd-color-picker')!;
    const trigger = colorPicker.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    const button = el.querySelector('button')!;
    const focusHandler = sinon.spy();
    const blurHandler = sinon.spy();

    colorPicker.addEventListener('gd-focus', focusHandler);
    colorPicker.addEventListener('gd-blur', blurHandler);
  ...
  it('should emit gd-focus when rendered inline and focused', async () => {
    const el = await fixture<SlColorPicker>(html`
      <div>
        <gd-color-picker inline></gd-color-picker>
        <button type="button">Click me</button>
      </div>
    `);
    const colorPicker = el.querySelector('gd-color-picker')!;
  ...
  it('should focus and blur when calling focus() and blur() and rendered as a dropdown', async () => {
    const colorPicker = await fixture<SlColorPicker>(html` <gd-color-picker></gd-color-picker> `);
    const focusHandler = sinon.spy();
    const blurHandler = sinon.spy();
  ...
  it('should focus and blur when calling focus() and blur() and rendered inline', async () => {
    const colorPicker = await fixture<SlColorPicker>(html` <gd-color-picker inline></gd-color-picker> `);
    const focusHandler = sinon.spy();
    const blurHandler = sinon.spy();
  ...
  describe('when submitting a form', () => {
    it('should serialize its name and value with FormData', async () => {
      const form = await fixture<HTMLFormElement>(html`
        <form>
          <gd-color-picker name="a" value="#ffcc00"></gd-color-picker>
        </form>
      `);
    ...
    it('should be present in form data when using the form attribute and located outside of a <form>', async () => {
      const el = await fixture<HTMLFormElement>(html`
        <div>
          ...
          <gd-color-picker form="f" name="a" value="#ffcc00"></gd-color-picker>
        </div>
      `);
  ...
  describe('when resetting a form', () => {
    it('should reset the element to its initial value', async () => {
      const form = await fixture<HTMLFormElement>(html`
        <form>
          <gd-color-picker name="a" value="#ffffff"></gd-color-picker>
          <gd-button type="reset">Reset</gd-button>
        </form>
      `);
      const button = form.querySelector('gd-button')!;
      const colorPicker = form.querySelector('gd-color-picker')!;
  ...
  describe('when using constraint validation', () => {
    ...
    it('should be invalid when required and empty', async () => {
      const el = await fixture<SlColorPicker>(html` <gd-color-picker required></gd-color-picker> `);
      expect(el.checkValidity()).to.be.false;
    });

    it('should be invalid when required and disabled is removed', async () => {
      const el = await fixture<SlColorPicker>(html` <gd-color-picker disabled required></gd-color-picker> `);
      el.disabled = false;
      await el.updateComplete;
      expect(el.checkValidity()).to.be.false;
    });
    ...
    it('should receive the correct validation attributes ("states") when valid', async () => {
      const el = await fixture<SlColorPicker>(html` <gd-color-picker required value="#fff"></gd-color-picker> `);
  ...
  runFormControlBaseTests('gd-color-picker');
});
