...
describe('<gd-switch>', () => {
  it('should pass accessibility tests', async () => {
    const el = await fixture<SlSwitch>(html` <gd-switch>Switch</gd-switch> `);
    await expect(el).to.be.accessible();
  });

  it('default properties', async () => {
    const el = await fixture<SlSwitch>(html` <gd-switch></gd-switch> `);
  ...
  it('should have title if title attribute is set', async () => {
    const el = await fixture<SlSwitch>(html` <gd-switch title="Test"></gd-switch> `);
  ...
  it('should be disabled with the disabled attribute', async () => {
    const el = await fixture<SlSwitch>(html` <gd-switch disabled></gd-switch> `);
  ...
  it('should emit gd-change when toggled with spacebar', async () => {
    const el = await fixture<SlSwitch>(html` <gd-switch></gd-switch> `);
    const changeHandler = sinon.spy();
    const inputHandler = sinon.spy();
  ...
  it('should emit gd-change and gd-input when toggled with the right arrow', async () => {
    const el = await fixture<SlSwitch>(html` <gd-switch></gd-switch> `);
    const changeHandler = sinon.spy();
    const inputHandler = sinon.spy();
  ...
  it('should emit gd-change and gd-input when toggled with the left arrow', async () => {
    const el = await fixture<SlSwitch>(html` <gd-switch checked></gd-switch> `);
    const changeHandler = sinon.spy();
    const inputHandler = sinon.spy();
  ...
  it('should not emit gd-change or gd-input when checked is set by JavaScript', async () => {
    const el = await fixture<SlSwitch>(html` <gd-switch></gd-switch> `);
    el.addEventListener('gd-change', () => expect.fail('gd-change incorrectly emitted'));
    el.addEventListener('gd-input', () => expect.fail('gd-change incorrectly emitted'));
    el.checked = true;
    await el.updateComplete;
    el.checked = false;
    await el.updateComplete;
  ...
  it('should hide the native input with the correct positioning to scroll correctly when contained in an overflow', async () => {
    //
    // See: https://github.com/gesdisc/components/issues/1169
    //
    const el = await fixture<SlSwitch>(html` <gd-switch></gd-switch> `);
  ...
  describe('when submitting a form', () => {
    it('should submit the correct value when a value is provided', async () => {
      const form = await fixture<HTMLFormElement>(html`
        <form>
          <gd-switch name="a" value="1" checked></gd-switch>
    ...
    it('should submit "on" when no value is provided', async () => {
      const form = await fixture<HTMLFormElement>(html`
        <form>
          <gd-switch name="a" checked></gd-switch>
          <gd-button type="submit">Submit</gd-button>
        </form>
      `);
      const button = form.querySelector('gd-button')!;
    ...
    it('should show a constraint validation error when setCustomValidity() is called', async () => {
      ...
      `);
      const button = form.querySelector('gd-button')!;
      const slSwitch = form.querySelector('gd-switch')!;
    ...
    it('should be invalid when required and unchecked', async () => {
      const slSwitch = await fixture<HTMLFormElement>(html` <gd-switch required></gd-switch> `);
      expect(slSwitch.checkValidity()).to.be.false;
    });

    it('should be valid when required and checked', async () => {
      const slSwitch = await fixture<HTMLFormElement>(html` <gd-switch required checked></gd-switch> `);
    ...
    it('should be present in form data when using the form attribute and located outside of a <form>', async () => {
      const el = await fixture<HTMLFormElement>(html`
        <div>
          ...
          <gd-switch form="f" name="a" value="1" checked></gd-switch>
        </div>
      `);
    ...
    it('should receive validation attributes ("states") even when novalidate is used on the parent form', async () => {
      const el = await fixture<HTMLFormElement>(html` <form novalidate><gd-switch required></gd-switch></form> `);
      const slSwitch = el.querySelector<SlSwitch>('gd-switch')!;
  ...
  describe('when resetting a form', () => {
    it('should reset the element to its initial value', async () => {
      ...
      `);
      const button = form.querySelector('gd-button')!;
      const switchEl = form.querySelector('gd-switch')!;
  ...
  it('should not jump the page to the bottom when focusing a switch at the bottom of an element with overflow: auto;', async () => {
    // https://github.com/gesdisc/components/issues/1169
    ...
    const switches = el.querySelectorAll<SlSwitch>('gd-switch');
  ...
  runFormControlBaseTests('gd-switch');
});
