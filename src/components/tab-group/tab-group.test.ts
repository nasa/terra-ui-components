...
const waitForScrollButtonsToBeRendered = async (tabGroup: SlTabGroup): Promise<void> => {
  await waitUntil(() => {
    const scrollButtons = tabGroup.shadowRoot?.querySelectorAll('gd-icon-button');
...
const expectOnlyOneTabPanelToBeActive = async (container: HTMLElement, dataTestIdOfActiveTab: string) => {
  await waitUntil(() => {
    const tabPanels = Array.from(container.getElementsByTagName('gd-tab-panel'));
...
describe('<gd-tab-group>', () => {
  ...
  it('displays all tabs', async () => {
    const tabGroup = await fixture<SlTabGroup>(html`
      <gd-tab-group>
        <gd-tab slot="nav" panel="general" data-testid="general-tab-header">General</gd-tab>
        <gd-tab slot="nav" panel="disabled" disabled data-testid="disabled-tab-header">Disabled</gd-tab>
        <gd-tab-panel name="general">This is the general tab panel.</gd-tab-panel>
  ...
  it('shows the first tab to be active by default', async () => {
    const tabGroup = await fixture<SlTabGroup>(html`
      <gd-tab-group>
        ...
        <gd-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</gd-tab-panel>
  ...
  describe('scrolling behavior', () => {
    const generateTabs = (n: number): HTMLTemplateResult[] => {
      ...
      for (let i = 0; i < n; i++) {
        result.push(
          html`<gd-tab slot="nav" panel="tab-${i}">Tab ${i}</gd-tab>
            <gd-tab-panel name="tab-${i}">Content of tab ${i}0</gd-tab-panel> `
        );
      }
      return result;
    };
    ...
    it('shows scroll buttons on too many tabs', async () => {
      const tabGroup = await fixture<SlTabGroup>(html`<gd-tab-group> ${generateTabs(30)} </gd-tab-group>`);
    ...
    it('does not show scroll buttons if all tabs fit on the screen', async () => {
      const tabGroup = await fixture<SlTabGroup>(html`<gd-tab-group> ${generateTabs(2)} </gd-tab-group>`);
    ...
    it.skip('does not show scroll buttons if placement is start', async () => {
      const tabGroup = await fixture<SlTabGroup>(html`<gd-tab-group> ${generateTabs(50)} </gd-tab-group>`);
      tabGroup.placement = 'start';
    ...
    it.skip('does scroll on scroll button click', async () => {
      ...
      const tabGroup = await fixture<SlTabGroup>(
        html`<gd-tab-group> ${generateTabs(numberOfElements)} </gd-tab-group>`
      );
      ...
      const scrollToRightButton = tabGroup.shadowRoot?.querySelector('gd-icon-button[part*="scroll-button--end"]');
  ...
  describe('tab selection', () => {
    const expectCustomTabToBeActiveAfter = async (tabGroup: SlTabGroup, action: () => Promise<void>): Promise<void> => {
      ...
      const showEventPromise = oneEvent(tabGroup, 'gd-tab-show') as Promise<SlTabShowEvent>;
      await action();
    ...
    const expectGeneralTabToBeStillActiveAfter = async (
      ...
      oneEvent(tabGroup, 'gd-tab-show').then(() => (showEventFired = true));
      oneEvent(tabGroup, 'gd-tab-hide').then(() => (hideEventFired = true));
      await action();
    ...
    it('selects a tab by clicking on it', async () => {
      const tabGroup = await fixture<SlTabGroup>(html`
        <gd-tab-group>
          <gd-tab slot="nav" panel="general" data-testid="general-header">General</gd-tab>
          <gd-tab slot="nav" panel="custom" data-testid="custom-header">Custom</gd-tab>
          <gd-tab-panel name="general">This is the general tab panel.</gd-tab-panel>
          <gd-tab-panel name="custom" data-testid="custom-tab-content">This is the custom tab panel.</gd-tab-panel>
        </gd-tab-group>
      `);
    ...
    it('does not change if a disabled tab is clicked', async () => {
      const tabGroup = await fixture<SlTabGroup>(html`
        <gd-tab-group>
          <gd-tab slot="nav" panel="general" data-testid="general-header">General</gd-tab>
          <gd-tab slot="nav" panel="disabled" data-testid="disabled-header" disabled>disabled</gd-tab>
          <gd-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</gd-tab-panel>
          <gd-tab-panel name="disabled">This is the disabled tab panel.</gd-tab-panel>
        </gd-tab-group>
      `);
    ...
    it('does not allow selection of disabled tabs with arrow keys', async () => {
      const tabGroup = await fixture<SlTabGroup>(html`
        <gd-tab-group>
          ...
          <gd-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</gd-tab-panel>
          <gd-tab-panel name="disabled">This is the custom tab panel.</gd-tab-panel>
        </gd-tab-group>
      `);
