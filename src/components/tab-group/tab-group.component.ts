 ...
 * @documentation https://disc.gsfc.nasa.gov/components/tab-group
 * @status stable
 * @since 2.0
 *
 * @dependency gd-icon-button
 *
 * @slot - Used for grouping tab panels in the tab group. Must be `<gd-tab-panel>` elements.
 * @slot nav - Used for grouping tabs in the tab group. Must be `<gd-tab>` elements.
 *
 * @event {{ name: String }} gd-tab-show - Emitted when a tab is shown.
 * @event {{ name: String }} gd-tab-hide - Emitted when a tab is hidden.
 *
 ...
 * @csspart scroll-button - The previous/next scroll buttons that show when tabs are scrollable, an `<gd-icon-button>`.
 ...
 */
export default class SlTabGroup extends GDElement {
  ...
  connectedCallback() {
    const whenAllDefined = Promise.all([
      customElements.whenDefined('gd-tab'),
      customElements.whenDefined('gd-tab-panel')
    ]);
  ...
  private getAllTabs(options: { includeDisabled: boolean } = { includeDisabled: true }) {
    ...
    return [...(slot.assignedElements() as SlTab[])].filter(el => {
      return options.includeDisabled
        ? el.tagName.toLowerCase() === 'gd-tab'
        : el.tagName.toLowerCase() === 'gd-tab' && !el.disabled;
    });
  }

  private getAllPanels() {
    return [...this.body.assignedElements()].filter(el => el.tagName.toLowerCase() === 'gd-tab-panel') as [SlTabPanel];
  }
  ...
  private handleKeyDown(event: KeyboardEvent) {
    ...
    if (['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Home', 'End'].includes(event.key)) {
      ...
      if (activeEl?.tagName.toLowerCase() === 'gd-tab') {
  ...
  updateScrollControls() {
    if (this.noScrollControls) {
      ...
      //
      // See https://github.com/gesdisc/components/issues/1839
