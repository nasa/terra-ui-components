...
describe('<gd-tree>', () => {
  ...
  beforeEach(async () => {
    el = await fixture(html`
      <gd-tree>
        <gd-tree-item>Node 1</gd-tree-item>
        <gd-tree-item>Node 2</gd-tree-item>
        <gd-tree-item id="expandable">
          Parent Node
          <gd-tree-item>Child Node 1</gd-tree-item>
          <gd-tree-item>
            Child Node 2
            <gd-tree-item>Child Node 2 - 1</gd-tree-item>
            <gd-tree-item>Child Node 2 - 2</gd-tree-item>
          </gd-tree-item>
        </gd-tree-item>
        <gd-tree-item>Node 3</gd-tree-item>
      </gd-tree>
    `);
  });
  ...
  describe('when a custom expanded/collapsed icon is provided', () => {
    ...
    it('should append a clone of the icon in the proper slot of the tree item', async () => {
      ...
      // Act
      const treeItems = [...el.querySelectorAll('gd-tree-item')];
  ...
  describe('Interactions', () => {
    ...
    describe('when selection is "single"', () => {
      describe('and user clicks on same item twice', () => {
        it('should emit `gd-selection-change` event once', async () => {
          // Arrange
          el.selection = 'single';
          await el.updateComplete;

          const selectedChangeSpy = sinon.spy();
          el.addEventListener('gd-selection-change', selectedChangeSpy);
  ...
  describe('when selection is "leaf"', () => {
    ...
    describe('and user clicks on expandable item', () => {
      it('should not emit `gd-selection-change` event', async () => {
        // Arrange
        el.selection = 'leaf';
        await el.updateComplete;
  ...
  describe('when selection is "multiple"', () => {
    describe('and user clicks on same item twice', () => {
      it('should emit `gd-selection-change` event twice', async () => {
        // Arrange
        el.selection = 'multiple';
        await el.updateComplete;
      ...
      describe('and a parent node is selected', () => {
        it('should select all the nested children', async () => {
          ...
          const tree = await fixture<SlTree>(html`
            <gd-tree selection="multiple">
              <gd-tree-item selected>
                Parent Node
                <gd-tree-item selected>Child Node 1</gd-tree-item>
                <gd-tree-item>
                  Child Node 2
                  <gd-tree-item>Child Node 2 - 1</gd-tree-item>
                  <gd-tree-item>Child Node 2 - 2</gd-tree-item>
          ...
          `);
          const treeItems = Array.from<SlTreeItem>(tree.querySelectorAll('gd-tree-item'));
  ...
  // https://github.com/gesdisc/components/issues/1916
  it("Should not render 'null' if it can't find a custom icon", async () => {
    const tree = await fixture<SlTree>(html`
      <gd-tree>
        <gd-tree-item>
          Item 1
          <gd-icon name="1-circle" slot="expand-icon"></gd-icon>
