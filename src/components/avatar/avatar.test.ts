...
describe('<gd-avatar>', () => {
  ...
  describe('when provided no parameters', () => {
    before(async () => {
      el = await fixture<SlAvatar>(html` <gd-avatar label="Avatar"></gd-avatar> `);
    });
  ...
  describe('when provided an image and label parameter', () => {
    ...
    before(async () => {
      el = await fixture<SlAvatar>(html`<gd-avatar image="${image}" label="${label}"></gd-avatar>`);
    });

    it('should pass accessibility tests', async () => {
      /**
       * The image element itself is ancillary, because it's parent container contains the
       * aria-label which dictates what "gd-avatar" is. This also implies that label text will
  ...
  describe('when provided initials parameter', () => {
    const initials = 'SL';
    before(async () => {
      el = await fixture<SlAvatar>(html`<gd-avatar initials="${initials}" label="Avatar"></gd-avatar>`);
    });
  ...
  describe('when image is present, the initials or icon part should not render', () => {
    ...
    before(async () => {
      el = await fixture<SlAvatar>(
        html`<gd-avatar image="${image}" label="${label}" initials="${initials}"></gd-avatar>`
      );
    });
  ...
  ['square', 'rounded', 'circle'].forEach(shape => {
    describe(`when passed a shape attribute ${shape}`, () => {
      before(async () => {
        el = await fixture<SlAvatar>(html`<gd-avatar shape="${shape}" label="Shaped avatar"></gd-avatar>`);
      });
  ...
  describe('when passed a <span>, on slot "icon"', () => {
    before(async () => {
      el = await fixture<SlAvatar>(html`<gd-avatar label="Avatar"><span slot="icon">random content</span></gd-avatar>`);
    });
  ...
  it('should not render the image when the image fails to load', async () => {
    el = await fixture<SlAvatar>(html`<gd-avatar></gd-avatar>`);
    el.image = 'bad_image';
