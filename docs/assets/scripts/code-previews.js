(() => {
  function convertModuleLinks(html) {
    html = html
      .replace(/@gesdisc\/components/g, `https://esm.sh/@gesdisc/components@${componentsVersion}`)
  ...
  const componentsVersion = document.documentElement.getAttribute('data-gesdisc-components-version');
  ...
  if (!componentsVersion) {
    throw new Error('The data-gesdisc-components-version attribute is missing from <html>.');
  }
  ...
  document.addEventListener('click', event => {
    ...
    if (button?.classList.contains('code-preview__button--codepen')) {
      ...
      const theme = document.documentElement.classList.contains('gd-theme-dark') ? 'dark' : 'light';
