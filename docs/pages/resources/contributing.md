...
Components must only emit custom events, and all custom events must start with `terra-` as a namespace. For compatibility with frameworks that utilize DOM templates, custom events must have lowercase, kebab-style names. For example, use `terra-change` instead of `slChange`.
...
When change events are emitted by Shoelace components, they should be named `terra-change` and they should only be emitted as a result of user input. Programmatic changes, such as setting `el.value = '…'` _should not_ result in a change event being emitted. This is consistent with how native form controls work.
...
Then use the following syntax for comments so they appear in the generated docs. Do not use the `--terra-` prefix, as that is reserved for design tokens that live in the global scope.
