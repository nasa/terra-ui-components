  ...
  plugins: [
          ...
          case ts.SyntaxKind.ClassDeclaration: {
            ...
            if (classDoc?.events) {
              classDoc.events.forEach(event => {
                event.reactName = `on${pascalCase(event.name)}`.replace(/^Gd/, 'Sl');
                event.eventName = `${pascalCase(event.name)}Event`.replace(/^Gd/, 'Sl');
              });
            }
          }
        }
    ...
    customElementVsCodePlugin({
      ...
      referencesTemplate: (_, tag) => [
        {
          name: 'Documentation',
          url: `https://disc.gsfc.nasa.gov/components/${tag.replace('gd-', '')}`
        }
      ]
    }),
    ...
    customElementVuejsPlugin({
      ...
      fileName: 'index.d.ts',
      componentTypePath: (_, tag) => `../../components/${tag.replace('gd-', '')}/${tag.replace('gd-', '')}.component.js`
    })
  ]
};
